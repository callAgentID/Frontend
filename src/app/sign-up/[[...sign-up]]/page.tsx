...
export default function SignUpPage() {
  ...
  const leftPanelBg = isLight ? 'rgba(220,234,252,0.60)' : 'rgba(4, 14, 36, 0.38)';
  const leftPanelBorder = isLight ? '1px solid rgba(26,111,212,0.12)' : '1px solid rgba(255,255,255,0.06)';
  ...
  const stepNumColor = isLight ? 'rgba(26,111,212,0.70)' : 'rgba(44,143,255,0.70)';
  const stepTitle = isLight ? '#0D1B2E' : '#EEF4FF';
  const stepDesc = isLight ? 'rgba(20,60,120,0.45)' : 'rgba(180,215,255,0.45)';
        ...
        <div className="relative space-y-8">
          <div className="space-y-5">
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 14px', borderRadius: 99, border: pillBorder, background: pillBg }}>
              ...
              <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: pillText }}>Get Started Free</span>
            </div>
            <h1 style={{ fontSize: '3.2rem', fontWeight: 900, lineHeight: 1.06, letterSpacing: '-0.03em', color: heroTitle, transition: 'color 0.3s ease' }}>
              Start analysing<br />
              <span style={{ color: heroAccent }}>smarter</span><br />
              today
            </h1>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            {[
              ...
              <div key={s.step} style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: stepNumColor, flexShrink: 0, marginTop: 2, letterSpacing: '0.04em' }}>{s.step}</span>
                <div>
                  <p style={{ fontSize: 13, fontWeight: 600, color: stepTitle, margin: 0, transition: 'color 0.3s ease' }}>{s.title}</p>
                  <p style={{ fontSize: 11, color: stepDesc, marginTop: 3, transition: 'color 0.3s ease' }}>{s.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      >
        ...
        <div style={{ width: '100%', maxWidth: 420 }}>
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: headingColor, letterSpacing: '-0.02em', margin: 0, transition: 'color 0.3s ease' }}>Create your account</h2>
            <p style={{ fontSize: 13, color: subtitleColor, marginTop: 6, transition: 'color 0.3s ease' }}>Start your intelligence journey today</p>
          </div>
